...
Page({
  data: {
    ...
    regionData: [], // 省市区数据
    pageScrollDisabled: false // 页面滚动禁用标志
  },
  ...
  // 选择居住地省市区（复用step1的地区选择器）
  selectResidenceArea() {
    logger.info('[居住地选择] 打开居住地省市区选择器');
    this.showResidenceRegionPicker();
  },
  ...
  goNext() {
    ...
    const savePromise = orderId 
      ? this.saveStep2ToServer(orderId).then(() => orderId)
      : this.createOrderAndSaveStep2();
  ...
  async saveStep2ToServer(orderId) {
    ...
    return req.post(`/public/orders/${orderId}/step2`, formData);
  },
  ...
  loadBorrowerInfoFromOrder(orderId) {
    ...
    req.request({
      ...
      const borrowerInfo = detail.borrowerInfo || {};
