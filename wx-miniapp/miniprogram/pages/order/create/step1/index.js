...
Page({
  data: {
    ...
    // 签约地省市区（省、市、区/县），与微信原生地区选择器联动
    // 例如：['广西壮族自治区', '南宁市', '青秀区']
    signRegion: [],
    // 地区选择器相关数据
    showRegionPicker: false,
    regionCurrentLevel: 'province', // province | city | district
    selectedProvince: '',
    selectedCity: '',
    selectedDistrict: '',
    ...
    regionSearchKeyword: '',
    regionSearchResults: [],
    regionProvinceGroups: [], // 按首字母分组的省份列表
    regionCityList: [], // 当前选中省份下的城市列表
    regionDistrictList: [], // 当前选中城市下的区县列表
    regionIndexLetters: [], // 首字母索引列表
    regionIndexActive: '', // 当前激活的首字母
    regionScrollIntoView: '', // 滚动定位目标
    // 省市区数据（静态数据，后续可从后端接口获取）
    regionData: [],
    // 页面滚动禁用标志
  ...
  onLoad(options) {
    try {
      ...
      if (mode !== 'view' && !orderId) {
        ...
        if (!formData.microloanOrg && this.data.microloanOrgOptions?.length > 0) {
          updates['formData.microloanOrg'] = this.data.microloanOrgOptions[0];
        }
        if (!formData.paymentChannel && this.data.paymentChannelOptions?.length > 0) {
          updates['formData.paymentChannel'] = this.data.paymentChannelOptions[0];
        }
        if (!formData.productType && this.data.productTypeOptions?.length > 0) {
          updates['formData.productType'] = this.data.productTypeOptions[0];
        }
        if (!formData.loanPurpose && this.data.loanPurposeOptions?.length > 0) {
          updates['formData.loanPurpose'] = this.data.loanPurposeOptions[0];
        }
        if (!formData.repaymentMethod && this.data.repaymentMethodOptions?.length > 0) {
          updates['formData.repaymentMethod'] = this.data.repaymentMethodOptions[0];
        }
        if (!formData.disputeResolution && this.data.disputeResolutionOptions?.length > 0) {
          updates['formData.disputeResolution'] = this.data.disputeResolutionOptions[0];
        }
        ...
        // 单选框：默认选中第一个
        if (!formData.borrowerType && this.data.loanTypeOptions?.length > 0) {
          updates['formData.borrowerType'] = this.data.loanTypeOptions[0].value;
        }
        if (!formData.notarization) {
          // “是否办理赋强公证”第一个选项是“是”
          updates['formData.notarization'] = '是';
        }
  ...
  loadLoanInfoFromOrder(orderId) {
    ...
    req.request({
      ...
      const loanInfo = detail.loanInfo || {};

      // 将后端 LoanInfo 字段映射到本页面的 formData 字段
      const formData = {
        assigneeOrg: loanInfo.assigneeOrg || '',
        microloanOrg: loanInfo.channelOrg || '',
        paymentChannel: loanInfo.payMethod || '',
        productType: loanInfo.productType || '',
        borrowerType: loanInfo.borrowerCategory || '',
        contractSignMode: loanInfo.contractSignMode || '',
        signMethod: loanInfo.contractSignMode || '在线签署',
        ...
        loanPurpose: loanInfo.usage || '',
        repaymentMethod: loanInfo.repayMode || '',
        disputeResolution: loanInfo.disputeWay || '',
        arbitrationOrg: loanInfo.arbitrationOrg || '',
        notarization: loanInfo.isNotarization || '',
        // 预留：后端暂未提供提前还款违约金比例字段
        penaltyRatio: this.data.formData.penaltyRatio || ''
      };

      this.setData({
        formData
      });
      logger.info('订单详情借款信息加载并回显成功:', loanInfo);
    }).catch(err => {
  ...
  showPicker(type) {
    ...
    switch(type) {
      ...
      case 'loanPurpose':
        ...
        title = '选择借款用途';
        currentValue = this.data.formData.loanPurpose;
        break;
      case 'repaymentMethod':
        options = this.data.repaymentMethodOptions;
        title = '选择还款方式';
        currentValue = this.data.formData.repaymentMethod;
        break;
      case 'disputeResolution':
        options = this.data.disputeResolutionOptions;
        title = '选择解决争议方式';
        currentValue = this.data.formData.disputeResolution;
        break;
      case 'arbitrationOrg':
  ...
  showRegionPicker() {
    ...
    // 如果已有选中的地区，初始化选择器状态
    const signPlace = this.data.formData.signPlace || '';
    ...
    if (signPlace) {
      // 先清理可能的逗号、空格等
      const cleanSignPlace = signPlace.replace(/[,，\s]/g, '');
      ...
      for (let i = 0; i < sortedProvinces.length; i++) {
        ...
        if (cleanSignPlace.startsWith(provinceName)) {
      ...
      if (matchedProvince) {
        ...
        const remaining = cleanSignPlace.substring(matchedProvinceIndex);
        ...
        const parts = cleanSignPlace.match(/(.*?[省市区县])/g) || [];
        if (parts.length >= 3) {
      ...
      // 调试日志
      logger.debug('showRegionPicker - 解析结果:', {
        signPlace,
        cleanSignPlace,
        matchedProvince,
        selectedProvince,
    ...
    this.setData({
      ...
      if (currentLevel === 'city' && selectedProvince) {
        ...
        logger.debug('showRegionPicker - 加载城市列表:', selectedProvince);
        this.loadCityList(selectedProvince);
      } else if (currentLevel === 'province') {
        // 如果没有已选省份，显示省份列表
        logger.debug('showRegionPicker - 初始化省份列表');
        this.initRegionData();
      }
    });
  },
  ...
  initRegionData() {
    // 使用微信小程序内置的地区数据
    // 如果微信小程序提供了 wx.getRegionData，可以使用它
    // 否则使用静态数据
    const provinces = this.getProvinceData();
  ...
  onRegionItemTap(e) {
    ...
    if (level === 'province') {
      ...
      if (!hasDistrictData) {
        ...
        const signPlace = `${province}${cityName}`;
        this.setData({
          selectedCity: cityName,
          selectedCityCode: code,
          selectedDistrict: '',
          'formData.signPlace': signPlace,
          signRegion: [province, cityName, '']
        });
      ...
      const signPlace = `${province}${city}${district}`;
      this.setData({
        selectedDistrict: district,
        selectedDistrictCode: code,
        'formData.signPlace': signPlace,
        signRegion: [province, city, district]
      });
      this.hideRegionPicker();
    }
  },
  ...
  /**
   * 加载城市列表（模拟数据，实际应从后端获取）
   */
  loadCityList(provinceName) {
    // 清理省份名称，去掉"省"、"市"、"自治区"、"特别行政区"等后缀，以便匹配
    ...
    // 调试日志
    logger.debug('loadCityList - 匹配结果:', {
      provinceName,
      provinceKey,
      citiesCount: cities.length,
      cityMapKeys: Object.keys(cityMap).slice(0, 10) // 只显示前10个key作为参考
    });
    
    if (cities.length === 0) {
      // 如果找不到匹配的城市，显示"暂无数据"
      logger.warn('loadCityList - 未找到匹配的城市:', { provinceName, provinceKey });
      this.setData({
        regionCityList: [
  ...
  /**
   * 检查并加载区县列表，如果没有区县数据则直接完成选择
   */
  checkAndLoadDistrictList(cityName) {
    const cityNameKey = cityName.replace('市', '').replace('特别行政区', '').replace('省', '');
    
    // 判断城市名是否已经包含完整信息（如"香港特别行政区"、"澳门特别行政区"、"台湾省"）
    const isCompleteCityName = cityName.includes('特别行政区') || cityName.includes('台湾省');
    
    if (isCompleteCityName) {
      // 如果城市名已经包含完整信息，直接完成选择，不显示区县层级
      const province = this.data.selectedProvince;
      const city = this.data.selectedCity;
      const signPlace = `${province}${city}`;
      this.setData({
        'formData.signPlace': signPlace,
        signRegion: [province, city, '']
      });
      this.hideRegionPicker();
      return;
    }
    ...
    // 检查是否有区县数据
    const hasDistrictData = this.hasDistrictData(cityNameKey);
    
    if (!hasDistrictData) {
      // 如果没有区县数据，直接完成选择（只有两层：省-市）
      const province = this.data.selectedProvince;
      const city = this.data.selectedCity;
      const signPlace = `${province}${city}`;
      this.setData({
      ...
      // 有区县数据，加载区县列表
      this.loadDistrictList(cityNameKey);
    }
  },
  ...
  /**
   * 加载区县列表（模拟数据，实际应从后端获取）
   */
  loadDistrictList(cityNameKey, hasSelectedDistrict = false) {
    // 尝试多种匹配方式
    const districtMap = this.getDistrictMap();
    // 清理城市名key，确保能正确匹配
    const cleanKey = cityNameKey.replace('市', '').replace('特别行政区', '').replace('省', '');
    // 尝试多种匹配方式：清理后的key、原始key、加上"市"、加上"特别行政区"
    ...
    // 如果是在重新打开时加载，且已经有selectedDistrict，说明之前选择过
    // 此时即使找不到数据，也不应该显示"暂无数据"，而是显示空列表（让用户知道需要重新选择）
    // 如果是在新选择时加载，且没有数据，才显示"暂无数据"
    // 同时检查 this.data.selectedDistrict 确保获取最新值
    const hasDistrict = hasSelectedDistrict || !!this.data.selectedDistrict;
    
    // 只有在没有数据且没有已选区级的情况下，才显示"暂无数据"
    // 如果已经有selectedDistrict（重新打开），即使找不到数据也不显示"暂无数据"
    if (districts.length === 0) {
      if (!hasDistrict) {
        // 新选择且没有数据，显示"暂无数据"
        districts = [
          { code: '000000', name: '暂无数据', fullName: '暂无数据', city: cityNameKey }
        ];
      }
      // 如果 hasDistrict 为 true（重新打开），即使没有数据也不显示"暂无数据"，保持空列表
    }
  ...
  onRegionSearchInput(e) {
    ...
    // 搜索省市区
    const results = this.searchRegion(keyword);
    this.setData({
  ...
  searchRegion(keyword) {
    ...
    // 搜索省份
    this.data.regionData.forEach(province => {
      if (province.name.includes(keyword) || province.pinyin.includes(keywordLower)) {
        results.push({
          code: province.code,
          name: province.name,
  ...
  scrollToRegionGroup(e) {
    ...
    // 对于特殊字符"*"，使用"star"作为id
    const scrollId = letter === '*' ? 'region-group-star' : `region-group-${letter}`;
    this.setData({
      regionScrollIntoView: scrollId,
   ...
   * 首字母索引触摸开始
   */
  onRegionIndexTouchStart(e) {
    const touch = e.touches[0];
    this.handleRegionIndexTouch(touch);
  },

  /**
   * 首字母索引触摸移动
   */
  onRegionIndexTouchMove(e) {
    const touch = e.touches[0];
    this.handleRegionIndexTouch(touch);
  },
   ...
   * 处理首字母索引触摸
   */
  handleRegionIndexTouch(touch) {
    const query = wx.createSelectorQuery();
    query.select('.region-index-bar').boundingClientRect(rect => {
      if (!rect) return;
      const { top, height } = rect;
      const y = touch.clientY - top;
      const itemHeight = height / this.data.regionIndexLetters.length;
      const index = Math.floor(y / itemHeight);
      const letter = this.data.regionIndexLetters[index];

      if (letter && letter !== this.data.regionIndexActive) {
        // 对于特殊字符"*"，使用"star"作为id
        const scrollId = letter === '*' ? 'region-group-star' : `region-group-${letter}`;
        this.setData({
          regionScrollIntoView: scrollId,
  ...
  selectLoanPurpose() {
    if (this.data.readonly) return;
    this.showPicker('loanPurpose');
  },

  // 选择还款方式
  selectRepaymentMethod() {
    if (this.data.readonly) return;
    this.showPicker('repaymentMethod');
  },

  // 选择解决争议方式
  selectDisputeResolution() {
    if (this.data.readonly) return;
    this.showPicker('disputeResolution');
  },
  ...
  validateForm() {
    ...
    const requiredFields = [
      ...
      { key: 'microloanOrg', name: '小贷机构' },
      { key: 'paymentChannel', name: '支付渠道' },
      { key: 'productType', name: '产品类型' },
      { key: 'borrowerType', name: '借款人类型' },
      { key: 'signMethod', name: '合同签署方式' },
      { key: 'loanAmount', name: '借款金额' },
      { key: 'loanDays', name: '借款天数' },
      { key: 'startDate', name: '借款起始日期' },
      { key: 'annualRate', name: '借款年利率' },
      { key: 'signPlace', name: '签约地' },
      { key: 'loanPurpose', name: '借款用途' },
      { key: 'repaymentMethod', name: '还款方式' },
      { key: 'disputeResolution', name: '解决争议方式' },
      { key: 'arbitrationOrg', name: '仲裁机构' },
      { key: 'notarization', name: '是否办理赋强公证' }
    ];
  ...
  goNext() {
    ...
    const savePromise = orderId 
      ? this.saveStep1ToServer(orderId, formData).then(() => orderId)
      : this.createOrderAndSaveStep1(formData);
  ...
  // 调用后端创建订单并保存步骤1数据（新建订单）
  createOrderAndSaveStep1(formData) {
    const req = wx.$request;
  ...
  saveStep1ToServer(orderId, formData) {
    const req = wx.$request;
    return req.post(`/public/orders/${orderId}/step1`, formData);
  },
