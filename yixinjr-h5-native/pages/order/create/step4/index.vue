    ...
    <scroll-view class="form-content" scroll-y enhanced :show-scrollbar="false">
      ...
      <view class="form-section">
        <view class="form-item">
          <text class="label required">是否有担保人</text>
          <view class="radio-group">
            <view class="radio-item" :class="{ active: hasGuarantor === true }" @tap="selectHasGuarantor(true)">
              <view class="radio-circle">
                <view class="radio-dot" v-if="hasGuarantor === true"></view>
              </view>
              <text>有</text>
            </view>
            <view class="radio-item" :class="{ active: hasGuarantor === false }" @tap="selectHasGuarantor(false)">
              <view class="radio-circle">
                <view class="radio-dot" v-if="hasGuarantor === false"></view>
              </view>
              <text>无</text>
            </view>
          </view>
      ...
      <view v-if="hasGuarantor">
        <view class="form-section">
          ...
          <!-- 担保类型 -->
          <picker :range="guaranteeTypeOptions" @change="onGuaranteeTypeChange">
            <view class="form-item">
              <text class="label required">担保类型</text>
              <view class="value-wrapper">
                <text class="value" :class="{ placeholder: !formData.guaranteeType }">
                  {{ formData.guaranteeType || '请选择担保类型' }}
                </text>
                <text class="arrow">›</text>
              </view>
            </view>
      ...
      <!-- 无担保人提示 -->
      <view v-else-if="hasGuarantor === false" class="no-data-tip">
        <text class="tip-icon">ℹ️</text>
        <text class="tip-text">无担保人，可直接进入下一步</text>
      </view>
    </scroll-view>
...
export default {
  data() {
    return {
      currentStep: 3,
      orderId: null,
      hasGuarantor: null, // null: 未选择, true: 有, false: 无
      ...
      guaranteeTypeOptions: ['连带责任保证', '一般保证', '抵押担保', '质押担保']
    };
  },
  ...
  methods: {
    ...
    loadOrderData() {
      ...
      req.request({
        ...
        const data = res.data || {};
        if (data.guarantor) {
          this.hasGuarantor = data.guarantor.hasGuarantor;
          if (this.hasGuarantor && data.guarantor.info) {
            this.formData = { ...this.formData, ...data.guarantor.info };
            // 检查现居地址是否与户籍地址相同
    ...
    onGuaranteeTypeChange(e) {
      this.formData.guaranteeType = this.guaranteeTypeOptions[e.detail.value];
    },
    ...
    validateForm() {
      ...
      const required = [
        ...
        { field: 'guaranteeType', label: '担保类型' },
    ...
    saveData() {
      ...
      const data = {
        hasGuarantor: this.hasGuarantor,
        info: this.hasGuarantor ? this.formData : null
      };
      
      return req.request({
        url: `/public/orders/${this.orderId}/guarantor-info`,
        method: 'PUT',
        data: data
      }).finally(() => {
        uni.hideLoading();
    ...
    goNext() {
      this.saveData().then(() => {
        ...
        setTimeout(() => {
          uni.navigateTo({
            url: `/pages/order/create/step5/index?id=${this.orderId}`
          });
        }, 1000);
      }).catch(err => {
